...
## -------------  GUI functions
    ...
    def populate_users(self,database_dir):
        '''
        List users in Station GUI based on the subfolders contained in [database_dir]
    ...
    def populate_instr(self,station):
        '''
        Function that lists all the components initialized in the qcodes station
    ...
    def InstrStart_callback(self):
        ...
        from source.gui.instruments import autoSF
        self.GUI = autoSF(self.station.components[self.actInstr],self.native_parameters_dict)
    ...
    def dataView_callback(self):
        ...
        if dataviewer_type == 'txt': # calls dataviewer for browsing ascii files generated by qtNE
            self.dataviewer=dataviewer_txt.DataViewer(data_directory = datadir)
        elif dataviewer_type == 'db': # call dataviewer for browsing qcodes database
