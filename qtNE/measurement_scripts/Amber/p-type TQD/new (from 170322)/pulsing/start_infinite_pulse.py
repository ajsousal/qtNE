...
def inf_pulse(instrument):
    ...
    twait = 100e-9 #600e-9#*10 for oscilloscope visualization  # first wait time (s)
    tblock= 200e-9#*10 for oscilloscope visualization
    tinter = 100e-9#*10 for oscilloscope visualization # 1e-6  # the second wait time (s) - time between pulses

    # Voltage on each gate 
    # WARNING: the input voltage is twice this value due to high input impedance (very resistive sample)
    high_level_RB = 1000* 1e-3 # 2000 * 1e-3# 2250 * 1e-3  # the high level (V)
