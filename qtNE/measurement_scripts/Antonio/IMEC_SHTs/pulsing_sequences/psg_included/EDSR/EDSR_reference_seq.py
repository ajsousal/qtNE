...
# using broadbeans
def sequence(pulser,mw,*args,**kwargs):

    # print(tburst)
    print('EDSR sequence 100ns MW, reference')
    ...
    SR = 1.2e9
    twait = 50e-9 #600e-9#*10 for oscilloscope visualization  # first wait time (s)
    tblock= 100e-9#*10 for oscilloscope visualization
    tinter = 50e-9#*10 for oscilloscope visualization # 1e-6  # the second wait time (s) - time between pulses

    # Voltage on each gate 
    # WARNING: the input voltage is twice this value due to high input impedance (very resistive sample)
    high_level_RB = 0 * 1e-3 #0 * 1e-3# 2250 * 1e-3  # the high level (V)
    ...
    baseshape_pulse.insertSegment(0, ramp, (0, 0), dur=twait)
    baseshape_pulse.insertSegment(1, ramp, (high_level_pulse, high_level_pulse), dur=tblock)
    baseshape_pulse.insertSegment(2, ramp, (0, 0), dur=tinter)
    ...
    seq.name = 'EDSR_sequence'  # the sequence name will be needed later
